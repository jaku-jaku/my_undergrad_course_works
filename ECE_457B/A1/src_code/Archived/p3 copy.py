...
class P3_Env:
    ...
    def plot_fitness_result(self, mlp, tag, use_test_data):
        plt.figure()
        fig2 = plt.gcf()
        
        if use_test_data:
            x = self._test_data_x
            y_true = self._test_data_y
        else:
    ...
    def evaluate_test_data(
        ...
        result = mlp.evaluate(self._test_data_x, self._test_data_y)
        return result
    ...
    def run_hyperParam_optimization(
        ...
        k_fold: int             = 10, # Default: 10-fold x-validation
        N_epoch: int            = 100, # Early stopping
        N_trial: int            = 5,
        plot: bool              = True
    ):
...
def main_env(f_func, tag, x_range, plot_progress):
    N_EPOCH_HARD_STOP = 1000 #epochs
    ...
    env.plot_fitness_result(mlp=best_mlp_new, tag="Best[Test Data]", use_test_data=True)
    env.plot_fitness_result(mlp=best_mlp_new, tag="Best[Overall]", use_test_data=False)
    env.plot_progress(hs=[h], tag="Best")
    ## hard evaluation:
    result = env.evaluate_test_data(mlp=best_mlp_new)
